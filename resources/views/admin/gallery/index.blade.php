...
<div class="flex items-center justify-between mb-6">
    <h2 class="text-lg font-bold text-secondary">Gallery Images</h2>
...
<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
    ...
    <div class="bg-white rounded-xl border border-secondary/5 shadow-sm overflow-hidden group">
        <div class="relative aspect-square overflow-hidden bg-surface">
            ...
            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                <a href="{{ route('admin.gallery.edit', $item) }}"
                   class="w-9 h-9 bg-white rounded-lg flex items-center justify-center text-secondary/70 hover:bg-primary hover:text-white transition-colors shadow-md"
                   title="Edit">
                ...
                <form action="{{ route('admin.gallery.destroy', $item) }}" method="POST"
                    ...
                    @csrf @method('DELETE')
                    <button type="submit"
                            class="w-9 h-9 bg-white rounded-lg flex items-center justify-center text-secondary/70 hover:bg-red-600 hover:text-white transition-colors shadow-md"
                            title="Delete">
        ...
        <div class="p-3">
            <p class="font-semibold text-secondary text-xs truncate">{{ $item->title }}</p>
            @if($item->category)
            <p class="text-[10px] text-primary font-medium mt-0.5">{{ $item->category }}</p>
            @endif
            <p class="text-[10px] text-secondary/50 mt-0.5">Order: {{ $item->sort_order }}</p>
        </div>
    </div>
    @endforeach
</div>
...
@else
<div class="bg-white rounded-xl border border-secondary/5 shadow-sm py-20 text-center">
    <div class="w-16 h-16 bg-surface rounded-2xl flex items-center justify-center mx-auto mb-4">
        <i class="fas fa-images text-secondary/50 text-2xl"></i>
    </div>
    <p class="text-secondary/50 font-medium text-sm">No gallery images yet</p>
